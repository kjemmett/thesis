...
Walter Fitch and Emanuel Margolish helped popularize this approach by constructing a weighted least squares approach to fitting phylogenetic trees from distance matrices \cite{Fitch:1967we}.
...
Finally, reticulate evolutionary processes are of more than just historical interest for evolutionary studies, but rather have a substantial 
...
First, we can think of $\beta_0$ as representing the number of connected components, or clusters, in our space.
Next, we can think of $\beta_1$ as representing the number of one-dimensional loops in our space.
...
Higher Betti numbers, $\beta_n$ for $n>1$ will correspond to higher dimensional holes.
In our coffee mug example, because both objects have the same Betti numbers ($\beta_0=1$, $\beta_1=1$, and $\beta_n=0$ for $n>1$), they are considered topologically equivalent.
...
Considering the shape of the tree, there is a single connected component, giving $\beta_0=1$.
Further, we see that there are no loops formed by the branches, giving $\beta_1=0$.
...
Considering the shape, there is again a single connected component, giving $\beta_0=1$.
However, because of the reticulate event mixing material from $a$ and $c$, there is now a loop formed in the topology, giving $\beta_1=1$
...
Darwin's branching phylogeny is a tree and hence trivially contractible ($\beta_n=0$ for $n>0$).
